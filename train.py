    ...
    for epoch in range(train_opt.epoch_count, train_opt.n_epochs + train_opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        for i, data in enumerate(train_dataset):
            ...
            for loop_i in range(1, len_loop):
                ...
                cur_loop_loss = loss_vtx_world + loss_vtx_cam + loss_rotmat
        ...
        if epoch % test_freq == 0:
            ...
            for j, data in enumerate(biwi_val_dataset):
                with torch.no_grad():
                    ...
                    align_domain['gt_vtx_world'].append(gt_face_world.cpu().numpy())
            ...
            # get aligned T
            # pred_t_aligned = get_aligned_prediction(gt_t, pred_t)  # [B, 3]

            # calc metric again
            pred_R_t = pred_R_t_tmp.copy()
            pred_R_t[:, :3, :3] = np.transpose(pred_R_aligned, (0, 2, 1))

            # pred_R_t = np.zeros([gt_R_t.shape[0], 4, 4], dtype=gt_R_t.dtype)
            # pred_R_t[:, :3, :3] = pred_R_aligned
            # pred_R_t[:, :3, 3] = pred_t_aligned
            # pred_R_t[:, 3, 3] = 1
            # pred_R_t = np.transpose(pred_R_t, (0, 2, 1))
