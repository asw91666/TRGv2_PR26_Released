...
class TRG(nn.Module):
    def __init__(self, cfg, init_face_full, init_pose):
        ...
        self.fc = BasicBlock(deconv_dim, deconv_dim, kernel_size=1)
    ...
    def forward(self, x, K_img=None, bbox_info=None, is_train=False):
        ...
        """
        batch_size, _, img_shape, _ = x.shape
        ...
        # Calc multi-scale feature
        for deconv_i in range(len(deconv_blocks)):
            s_feat = deconv_blocks[deconv_i](s_feat)
            ms_feat_list.append(s_feat.clone())
        out_list['features'] = ms_feat_list

        # estimate 2d coordinate
        ...
        for rf_i in range(self.cfg.MODEL.TRG.N_ITER):
            pred_pose = output['pred_pose'].detach() # [B,9]
            pred_face = output['pred_face_world'].detach() # [B,1220,3]
            if rf_i > 0:
                pred_R_t = output['pred_R_t'].detach() # [B,4,4]

            # s_feat_i = ms_feat_list[rf_i]
            if rf_i == 0:
                s_feat_i = ms_feat_list[0]
            # s_feat = s_feat_i
