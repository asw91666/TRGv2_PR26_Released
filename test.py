...
if __name__ == '__main__':
    ...
    for i, data in enumerate(dataset):
        ...
        with torch.no_grad():
            ...
            gt_vtx_img = data['vtx_img'].to(device)  # [B,1220,3]
            ...
            intrinsic_full = data['K'].to(device)  # [B,4,3]
            bbox_info = data['bbox_info'].to(device)  # [B,5]
            align_mat = data['align_mat'].to(device)  # [B,4,4]
            ...
            ##############################################################################################
            align_domain['gt_R_t'].append(gt_extrinsic.cpu().numpy())
            align_domain['pred_R_t'].append(pred_R_t.cpu().numpy())
            align_domain['gt_vtx_world'].append(gt_vtx_world.cpu().numpy())
            align_domain['transl_bias'].append(align_mat.cpu().numpy())
            ...
            ##############################################################################################
            # calc head to camera distance
            ##############################################################################################
            face_to_cam_dist = np.linalg.norm(gt_t, axis=-1) # [B]
            result['distance'].append(face_to_cam_dist)
            ##############################################################################################
            # calc 3DRecon error
            ##############################################################################################
            ...
            ##############################################################################################
            # calc 2d face landmark error (NME)
            ##############################################################################################
            if opt.dataset_mode == 'arkit':
                # pred vtx
                ones = torch.ones([batch_size, pred_vtx_world.shape[1], 1], device=pred_vtx_world.device)
                pred_vtx_world_homo = torch.cat([pred_vtx_world, ones], dim=2) # [B,1220,4]
                vtx_cam = torch.bmm(pred_vtx_world_homo, gt_extrinsic)
                tmp = torch.bmm(vtx_cam, intrinsic_full)
                vtx_img = tmp[:, :, :2] / tmp[:, :, [2]] # [B,nv,2]
                vtx_img_np = vtx_img.cpu().numpy()

                # get 68 lmk gt
                gt_vtx_img_np = gt_vtx_img.cpu().numpy()
                gt_lmk68_np = gt_vtx_img_np[:, kpt_ind, :2] # [B,68,2]

                # 1. 정규화 인자 구하기 (배치 크기 [32]의 배열 반환)
                norm_factors = get_interocular_distance(gt_lmk68_np)

                # 2. 배치 NME 계산 (배치 크기 [32]의 NME 배열 반환)
                nme_cur_batch = calculate_nme(gt_vtx_img_np[..., :2], vtx_img_np, norm_factors)
                result['nme'].append(nme_cur_batch)
    ...
    if opt.dataset_mode == 'arkit':
        # load function
        distance = np.concatenate(result['distance'], axis=0)
        geometry_mean_error = np.concatenate(result['3DRecon'], axis=0)
        final_balanced_error, debug_info = calculate_balanced_distance_error(distance, geometry_mean_error, num_bins=30)
        result['b_mean'].append(final_balanced_error)
        print(f'debug_info: \n{debug_info}')
    ...
    if opt.dataset_mode == 'biwi':
        ########################################################################
        # alignment reference system
        ########################################################################
        for k in align_domain.keys():
            try:
                align_domain[k] = np.concatenate(align_domain[k], axis=0)
            except:
                print(f'error : {k}')

        # load data
        gt_vtx_world = align_domain['gt_vtx_world']  # [B,6918,4]
        gt_R_t = align_domain['gt_R_t']  # [B,4,4]
        pred_R_t_tmp = align_domain['pred_R_t']  # [B,4,4]
        transl_bias = align_domain['transl_bias'] # [B,4,4]
        pred_R_t_tmp = transl_bias @ pred_R_t_tmp
        # get R
        gt_R = np.transpose(gt_R_t[:, :3, :3], (0, 2, 1))  # [B,3,3]
        pred_R = np.transpose(pred_R_t_tmp[:, :3, :3], (0, 2, 1))  # [B,3,3]

        # get aligned R
        evaluator = Evaluator(gt_R, pred_R)  # matrix 는 [B,3,3] 으로 입력한다.

        # Compute error after using alignment algorithm
        pred_R_aligned = evaluator.align_predictions()

        pred_R_t = pred_R_t_tmp.copy()
        pred_R_t[:, :3, :3] = np.transpose(pred_R_aligned, (0, 2, 1))

        # prepare
        pred_R_t = torch.from_numpy(pred_R_t).cuda()
        gt_extrinsic = torch.from_numpy(gt_R_t).cuda()
        gt_vtx_world = torch.from_numpy(gt_vtx_world).cuda()
        ...
        _, ge_err = calc_geodesic_error(pred_R, gt_R)
        result['ge_err_al'].append(ge_err)  # batch_size

        result['r1_err_al'].append(r1_err)  # batch_size
        result['r2_err_al'].append(r2_err)
        result['r3_err_al'].append(r3_err)

        result['yaw_mae_al'].append(yaw_mae)  # batch_size
        result['pitch_mae_al'].append(pitch_mae)
        result['roll_mae_al'].append(roll_mae)

        result['tx_mae_al'].append(tx_mae)  # batch_size
        result['ty_mae_al'].append(ty_mae)
        result['tz_mae_al'].append(tz_mae)

        pred_vtx_cam_ADD = torch.bmm(gt_vtx_world, pred_R_t)[:, :, :3].cpu().numpy()  # [B,1220,3]
        gt_vtx_cam_ADD = torch.bmm(gt_vtx_world, gt_extrinsic)[:, :, :3].cpu().numpy()  # [B,1220,3]
        cur_ADD = calc_keypoint_mae(pred_vtx_cam_ADD, gt_vtx_cam_ADD)
        result['ADD_al'].append(cur_ADD)  # batch_size
